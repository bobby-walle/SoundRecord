...
public class AudioActivity extends Activity implements View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        File fpath = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/audiofiles/");
        boolean mkdirs = fpath.mkdirs();//创建文件夹
        try {
            //创建临时文件,注意这里的格式为.pcm
...
//                short[] buffer = new short[bufferSize];
                ...
                int audioBufSize = AudioRecord.getMinBufferSize(AudioConfig.SAMPLERATE,
                        AudioConfig.RECORDER_CHANNEL_CONFIG, AudioConfig.AUDIO_FORMAT);
                byte[] samples = new byte[audioBufSize];
                // 初始化recorder
                if (null == audioRecord) {
                ...
                while (isRecording) {
                    ...
                    int bufferReadResult = audioRecord.read(samples, 0, 320);
